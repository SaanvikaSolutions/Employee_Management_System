document.getElementById('searchBar').addEventListener('keyup', function () {
    ...
    rows.forEach(row => {
        ...
        const expenseType = row.cells[2].textContent.toLowerCase();
        const expenseCategory = row.cells[3].textContent.toLowerCase();
        const projectType = row.cells[5].textContent.toLowerCase();
        const projectName = row.cells[6].textContent.toLowerCase();
