...
if (projectType === 'Interior') {
    if (propertyType.includes('Flat') || propertyType.includes('Villa') || propertyType.includes('Apartment') || propertyType.includes('Individual House') || propertyType.includes('Office')) {
        if (propertyType.includes('Office')) {
            // Show office options instead of BHK options
            showOfficeOptions(); // Options: Small Office, Medium Office, Large Office, Customize
        } else {
            showDefaultOptions(); // Show BHK options for other property types
        }
    }
} else if (projectType === 'Construction') {
    // If the selected property type is Office, show office options
    if (propertyType.includes('Office')) {
        showOfficeOptions(); // Show office options for Construction project type
    } else {
...
let selectedPropertyType = '';  // To store the type of property selected (e.g., 'residential', 'office')
let selectedProjectType = '';   // To store the selected project type (Interior or Construction)
let selectedRooms = {};        // To store rooms and their counts
...
function selectProperty(element, propertyType) {
    ...
    // Add or remove the selected property type from the selected options
    displaySelectedOptions();

    // Show room options based on selected property type
    if (propertyType === 'residential') {
        showDefaultOptions();  // Show room options for Residential
    } else if (propertyType === 'office') {
        showOfficeOptions();  // Show room options for Office
    }
}

// Function to show default residential options (e.g., BHK options)
function showDefaultOptions() {
    const defaultOptions = ['1BHK', '2BHK', '3BHK', '4BHK', '5BHK', '6BHK', '7BHK', '8BHK', '9BHK'];
    const container = document.getElementById('dynamic-options-container');
    container.style.display = 'flex';
    ...
    defaultOptions.forEach(option => {
        ...
        button.onclick = () => {
            toggleSelection(button); // Highlight selection
            toggleRoomSelection(option); // Show room selection for the selected option
        };
        container.appendChild(button);
    });
}

// Function to show office options
function showOfficeOptions() {
    const officeOptions = ['Small Office', 'Medium Office', 'Large Office', 'Customize'];
    const container = document.getElementById('dynamic-options-container');
    container.style.display = 'flex';
...
// Function to track room selection and show room details
function toggleRoomSelection(option) {
    const roomOptionsContainer = document.getElementById('room-options');
    roomOptionsContainer.innerHTML = '';  // Clear previous room options
    document.getElementById('room-selection-container').style.display = 'flex';  // Ensure the room selection container is visible
...
// Create room option with count buttons
function createRoomOption(roomName, imageUrl) {
    const roomOption = document.createElement('div');
    roomOption.className = 'room-option';
    roomOption.onclick = () => toggleRoom(roomOption, roomName);
    roomOption.innerHTML = `
        <img src="${imageUrl}" alt="${roomName}">
        <span>${roomName}</span>
        <div>
            <button class="minus-button" onclick="changeCount(event, this, -1, '${roomName}')">-</button>
            <span class="count">0</span>
            <button class="plus-button" onclick="changeCount(event, this, 1, '${roomName}')">+</button>
        </div>
    `;
    return roomOption;
}

// Function to update room count in selectedRooms object
function changeCount(event, element, increment, roomName) {
    // Prevent triggering toggleRoom when clicking on count buttons
    event.stopPropagation();
    ...
    // Update selectedRooms object
    if (count > 0) {
        selectedRooms[roomName] = count;
    } else {
        delete selectedRooms[roomName];
    }
...
// Function to display the selected property type and rooms
function displaySelectedOptions() {
    const selectedContainer = document.getElementById('selected-options-container');
    selectedContainer.innerHTML = '';  // Clear current selection

    // Show selected property type
    if (selectedPropertyType) {
        const propertyDiv = document.createElement('div');
        propertyDiv.className = 'selected-option';
        propertyDiv.innerHTML = `
            <span>Property Type: ${selectedPropertyType}</span>
            <button class="remove-btn" onclick="removeOption('property')">Remove</button>
        `;
        selectedContainer.appendChild(propertyDiv);
    }

    // Show selected project type
    if (selectedProjectType) {
        const projectDiv = document.createElement('div');
        projectDiv.className = 'selected-option';
        projectDiv.innerHTML = `
            <span>Project Type: ${selectedProjectType}</span>
            <button class="remove-btn" onclick="removeOption('project')">Remove</button>
        `;
        selectedContainer.appendChild(projectDiv);
    }

    // Show rooms and counts
    for (const room in selectedRooms) {
        if (selectedRooms[room] > 0) {
            const roomDiv = document.createElement('div');
            roomDiv.className = 'selected-option';
            roomDiv.innerHTML = `
                <span>${room}: ${selectedRooms[room]}</span>
                <button class="remove-btn" onclick="removeRoom('${room}')">Remove</button>
            `;
            selectedContainer.appendChild(roomDiv);
        }
    }
}

// Function to remove a selected property or project type
function removeOption(type) {
    if (type === 'property') {
        selectedPropertyType = '';  // Clear selected property type
    } else if (type === 'project') {
        selectedProjectType = '';  // Clear selected project type
    }
    displaySelectedOptions(); // Re-render the selected options
}

// Function to remove a room selection
function removeRoom(roomName) {
    delete selectedRooms[roomName]; // Remove the selected room
    displaySelectedOptions(); // Re-render the selected options
}
...
function showDynamicOptions(projectType, element) {
    ...
    // Add or remove the selected project type from the selected options
    displaySelectedOptions();
}


// ----------------------- 2nd code------------------// ----------------------- 2nd code------------------// Your data structure for additional options
...
function selectOption(element, type) {
    ...
    // Always show dimension calculator if any room selection is made
    dimensionCalculator.style.display = 'block';  // Show dimension calculator as well

    // Hide material type section initially (it will only appear after selecting an additional option)
    roomSizeOptions.style.display = 'none';
}
...
function selectAdditionalOption(option) {
    ...
    // Show Material Type options after selecting an additional option
    const roomSizeOptions = document.getElementById('roomSizeOptions');
    roomSizeOptions.style.display = 'block';  // Show Material Type section
...
function selectRoomSize(button, size) {
    ...
    roomSizeButtons.forEach(btn => btn.classList.remove('selected'));  // Remove 'selected' class from all buttons
    button.classList.add('selected');  // Add 'selected' class to the clicked button
    console.log(`You’ve selected the room size: ${size}`);  // Correct string interpolation using backticks

    // Example additional functionality:
    checkForQuotation();  // This can be any other function you want to call
}
...
// Call calculateArea on page load
