...
// Track selected categories and options
let selectedCategories = [];  // To store selected categories
let selectedOptions = [];     // To store selected options with their levels

// Function to toggle selection for each type (category)
function selectOption(element, type) {
    ...
    if (additionalOptionsData[type]) {
        if (isSelected) {
            selectedCategories.push(type);  // Add to selected categories list

            // Populate additional options if they exist for the selected type
            additionalContent.innerHTML = additionalOptionsData[type]
                .map((option, index) => `
                    <div class="EMS-additional-option" onclick="selectAdditionalOption(${index}, '${type}')">
            ...
            selectedCategories = selectedCategories.filter(cat => cat !== type);  // Remove from selected categories
...
function selectAdditionalOption(index, category) {
    const option = additionalOptionsData[category][index];
    
    // Toggle the selection state of the option
    const selectedOptionIndex = selectedOptions.findIndex(opt => opt.text === option.text && opt.category === category);
    
    if (selectedOptionIndex === -1) {
        selectedOptions.push({
            text: option.text,
            category: category,
            level: 'Basic'  // Default level is Basic
        });
    } else {
        selectedOptions.splice(selectedOptionIndex, 1);  // Deselect if it's already selected
    }

    updateSelectedOptionsDisplay();
    showLevelSelection(option.text, category);
}

// Function to show level selection (Basic, Premium, Luxury) for each option
function showLevelSelection(optionText, category) {
    const levelSelectionDiv = document.getElementById('levelSelection');
    levelSelectionDiv.innerHTML = `
        <div>
            <span>Select a level for "${optionText}":</span>
            <button onclick="setOptionLevel('${optionText}', '${category}', 'Basic')">Basic</button>
            <button onclick="setOptionLevel('${optionText}', '${category}', 'Premium')">Premium</button>
            <button onclick="setOptionLevel('${optionText}', '${category}', 'Luxury')">Luxury</button>
        </div>
    `;
}

// Function to set the selected level for an option
function setOptionLevel(optionText, category, level) {
    const option = selectedOptions.find(opt => opt.text === optionText && opt.category === category);
...
// Function to update and display all selected options
function updateSelectedOptionsDisplay() {
    const selectedOptionsDiv = document.getElementById('selectedOptions');
    selectedOptionsDiv.innerHTML = selectedOptions
        .map(opt => `
            <div>
                <span>${opt.category} - ${opt.text} - Level: ${opt.level}</span>
            </div>
        `).join('');
}
